...
# Custom agents in .codex/agents/ are allowed (the Architect creates them).
...
# ── AGENTS.md: never modify at runtime ──────────────────────────────────────
if [[ "$BASENAME" == "AGENTS.md" && "$FILE" != *".codex/"* ]]; then
  echo "BLOCKED: AGENTS.md is a system file. Update it in the repo and run updateme.sh."
  exit 2
fi
