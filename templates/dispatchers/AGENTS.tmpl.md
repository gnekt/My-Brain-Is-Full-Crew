...
Your crew consists of **14 skills** (in `{{PLATFORM_DIR}}/skills/`) and **8 core agents** (in `{{PLATFORM_DIR}}/agents/`). {{PLATFORM_NAME}} auto-loads both at session start.

The 8 core agents are:

`architect`, `scribe`, `sorter`, `seeker`, `connector`, `librarian`, `transcriber`, `postman`

Custom agents created by the Architect are also valid. Check `{{PLATFORM_DIR}}/references/agents-registry.md` for the full list of active agents (core + custom).
...
- **Skills** handle complex, multi-step, or conversational flows. Invoke them via the **{{TOOL_SKILL}} tool**. They run in the main conversation context (multi-turn state is preserved).
- **Agents** handle reactive, single-shot operations. Invoke them via the **{{TOOL_AGENT}} tool**. They run as subprocesses.
...
2. **Check skill routing FIRST** — if the user's message matches a skill trigger, invoke the skill using the **{{TOOL_SKILL}} tool**. Do NOT use the {{TOOL_AGENT}} tool for skill-routed triggers.
3. **Fall through to agent routing** — if NO skill matches, use the agent routing table and invoke via the **{{TOOL_AGENT}} tool**.
4. **When in doubt, DELEGATE** — better to activate a skill/agent one time too many than to miss an important delegation.
5. **Pass the user's message** — in the {{TOOL_AGENT}}/{{TOOL_SKILL}} prompt, include the user's original message as-is.

---

## Skill routing (check FIRST — highest priority)

Skills handle complex, multi-step flows. **Check this table BEFORE the agent table.** If a match is found, invoke the skill via the `{{TOOL_SKILL}}` tool and STOP — do not also invoke an agent.
...
| 9+ | **custom agents** | Any agent created via the Architect. Check `{{PLATFORM_DIR}}/references/agents-registry.md` for triggers and capabilities. Custom agents always have lower priority than core 8. |
...
Custom agents are created via the `/create-agent` skill and stored in `{{PLATFORM_DIR}}/agents/`. They are auto-discovered by {{PLATFORM_NAME}} like core agents. When a user message does not match any skill or core agent, check `{{PLATFORM_DIR}}/references/agents-registry.md` for custom agents whose Input column matches the message. If a match is found, delegate to that agent.
...
Consult `{{PLATFORM_DIR}}/references/agents-registry.md` to validate suggestions and match output to agent capabilities.
...
See `{{PLATFORM_DIR}}/references/agent-orchestration.md` for the full protocol and `{{PLATFORM_DIR}}/references/agents-registry.md` for the agent registry.
...
The script asks a couple of questions and copies everything into `{{PLATFORM_DIR}}/` inside your vault:

```
your-vault/
├── {{PLATFORM_DIR}}/
│   ├── agents/          ← 8 crew agents (auto-loaded by {{PLATFORM_NAME}})
...
1. Open {{PLATFORM_NAME}} **inside your vault folder**
...
- **{{PLATFORM_NAME}}** with a Claude Pro, Max, or Team subscription
...
Each agent is defined in `{{PLATFORM_DIR}}/agents/{name}.md` (in the destination vault) with YAML frontmatter (`name`, `description`, `tools`, `model`) and a full system prompt body. {{PLATFORM_NAME}} auto-discovers these agents at session start, reads their `description` field, and delegates automatically when the user's message matches.

The {{DISPATCHER_FILE}} routing rules REINFORCE this auto-delegation — they provide explicit priority ordering and trigger lists to ensure Claude delegates correctly.

Key design decisions:

- **Seeker** is search-only (`tools: {{TOOL_READ}}, {{TOOL_GLOB}}, {{TOOL_GREP}}`) — it finds information but doesn't modify notes
- **Architect** and **Librarian** have full access including {{TOOL_BASH}} for structural operations
...
- Agents reference shared docs at `{{PLATFORM_DIR}}/references/`
...
This loads agents + MCP for the current session. You still need to run `launchme.sh` to set up `{{PLATFORM_DIR}}/references/` in the vault.
