...
You MUST use the `{{TOOL_ASK}}` tool for EVERY question in every phase. This is not optional. This is how the onboarding works:
...
1. Ask ONE question using `{{TOOL_ASK}}`
...
4. Ask the NEXT question using `{{TOOL_ASK}}`
...
- **ONE question per `{{TOOL_ASK}}` call.** Never bundle 2+ questions in one message.
...
- **NEVER output all questions as text.** The questions below are for YOU to ask one at a time via `{{TOOL_ASK}}`, not to display as a list.
...
This step ensures the crew agents activate **only when {{PLATFORM_NAME}} is opened in this vault** — not in other projects or coding sessions.

Use Bash to:

```bash
# 1. Create the project-scoped agents directory inside the vault
mkdir -p {{PLATFORM_DIR}}/agents

# 2. Find where the crew agent files are currently installed
# Try user-scope location first, then common plugin cache paths
AGENT_SOURCE=""
if ls ~/{{PLATFORM_DIR}}/agents/architect.md 2>/dev/null; then
  AGENT_SOURCE=~/{{PLATFORM_DIR}}/agents
fi

# 3. Copy only the agents the user selected during onboarding
# (copy all if the user selected "all agents")
if [ -n "$AGENT_SOURCE" ]; then
  cp "$AGENT_SOURCE"/architect.md {{PLATFORM_DIR}}/agents/
  # Copy each selected agent — replace the list based on Phase 2 answers:
  # cp "$AGENT_SOURCE"/scribe.md {{PLATFORM_DIR}}/agents/
  # cp "$AGENT_SOURCE"/sorter.md {{PLATFORM_DIR}}/agents/
  # cp "$AGENT_SOURCE"/seeker.md {{PLATFORM_DIR}}/agents/
  # cp "$AGENT_SOURCE"/connector.md {{PLATFORM_DIR}}/agents/
  # cp "$AGENT_SOURCE"/librarian.md {{PLATFORM_DIR}}/agents/
  # cp "$AGENT_SOURCE"/transcriber.md {{PLATFORM_DIR}}/agents/
  # cp "$AGENT_SOURCE"/postman.md {{PLATFORM_DIR}}/agents/
fi
```

After copying, verify with `ls {{PLATFORM_DIR}}/agents/` that the files are in place.

**If the agent source cannot be found automatically**, tell the user:
> "I couldn't find the crew agent files automatically. Please copy the `.md` files from the `agents/` folder of the plugin into `{{PLATFORM_DIR}}/agents/` inside your vault. I've created the folder for you — it's at `[vault path]/{{PLATFORM_DIR}}/agents/`."

**B2. Verify reference files**

The crew agents read shared docs from `{{PLATFORM_DIR}}/references/`. The `launchme.sh` script copies these automatically. Verify they exist:

```bash
ls {{PLATFORM_DIR}}/references/agents.md {{PLATFORM_DIR}}/references/agent-orchestration.md {{PLATFORM_DIR}}/references/agents-registry.md
```

If they don't exist, create them from scratch using Write:
- `{{PLATFORM_DIR}}/references/agents.md` — one paragraph per agent describing its role and vault area
- `{{PLATFORM_DIR}}/references/agent-orchestration.md` — the inter-agent coordination protocol (dispatcher-driven)
- `{{PLATFORM_DIR}}/references/agents-registry.md` — the single source of truth for all agents (supports core + custom agents)
...
>
> The agents are installed in `{{PLATFORM_DIR}}/agents/` inside your vault. This means:
> - When you open {{PLATFORM_NAME}} in this vault folder, all your crew agents activate
> - When you open {{PLATFORM_NAME}} in any other project, no crew agents
>
> **One thing to check:** if you installed the plugin as a 'Personal plugin' in {{PLATFORM_NAME}} Desktop, the agents will also be available in all your other projects. To keep things clean, you can remove it from Personal plugins — your vault now has its own local copy that takes priority anyway."
...
Read `{{PLATFORM_DIR}}/references/templates.md` for the full set of template definitions. If that file does not exist, create templates based on these specifications:
...
After creating the vault structure, scope the crew agents to this vault only by copying them into `{{PLATFORM_DIR}}/agents/` inside the vault. Only copy the agents the user selected during Phase 2 (Q7). The Architect is always copied.

After copying, verify with `ls {{PLATFORM_DIR}}/agents/` that the files are in place.

If the agent source cannot be found automatically, instruct the user to copy the `.md` files manually from the `agents/` folder of the plugin into `{{PLATFORM_DIR}}/agents/` inside their vault.

Also verify that `{{PLATFORM_DIR}}/references/` contains the shared docs (`agents.md`, `agent-orchestration.md`, `agents-registry.md`). If missing, create them.
...
[ ] {{PLATFORM_DIR}}/agents/ has the selected agent files
[ ] {{PLATFORM_DIR}}/references/ has shared docs
