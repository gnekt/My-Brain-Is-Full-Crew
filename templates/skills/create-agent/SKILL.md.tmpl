...
**Before starting, read `{{PLATFORM_DIR}}/references/agent-template.md`** to understand the standard structure every agent must follow.
...
You MUST use the `{{TOOL_ASK}}` tool for EVERY question in every phase. This is not optional. This is how the conversation works:
...
- **ONE question per `{{TOOL_ASK}}` call.** Never bundle 2+ questions.
...
4. **Does this agent need to create or modify notes?** Based on the answer:
   - Read-only: tools = `{{TOOL_READ}}, {{TOOL_GLOB}}, {{TOOL_GREP}}`
   - Creates notes: tools = `{{TOOL_READ}}, {{TOOL_WRITE}}, {{TOOL_GLOB}}, {{TOOL_GREP}}`
   - Modifies existing notes: tools = `{{TOOL_READ}}, {{TOOL_WRITE}}, {{TOOL_EDIT}}, {{TOOL_GLOB}}, {{TOOL_GREP}}`
...
3. **Generate the agent file** following `{{PLATFORM_DIR}}/references/agent-template.md`:
...
4. **Save the file** to `{{PLATFORM_DIR}}/agents/{name}.md`
5. **Update the registry**: add a new row to `{{PLATFORM_DIR}}/references/agents-registry.md`
6. **Update the directory**: add a new section under "Custom Agents" in `{{PLATFORM_DIR}}/references/agents.md`
