...
For the definitive list of agents with capabilities, inputs, outputs, and status, see `{{PLATFORM_DIR}}/references/agents-registry.md`. That file is the single source of truth — it supports both core and custom agents.
...
When an agent detects work for another agent, it includes a `### Suggested next agent` section in its output. The dispatcher reads this and decides whether to chain the next agent. See `{{PLATFORM_DIR}}/references/agent-orchestration.md` for the full protocol.
...
Custom agents are created by the Architect and live in `{{PLATFORM_DIR}}/agents/` alongside the core agents. They follow the same conventions: YAML frontmatter, trigger phrases written in the user's language, inter-agent coordination sections, and dispatcher-driven orchestration.

For the definitive list of all agents (core + custom) with capabilities, inputs, outputs, and status, see `{{PLATFORM_DIR}}/references/agents-registry.md`.
