...
This document defines how agents coordinate through the **dispatcher** (`{{DISPATCHER_FILE}}`). Agents do NOT communicate directly with each other — the dispatcher handles all routing and chaining.
...
2. **Skill match found?** → invoke the skill via the **{{TOOL_SKILL}}** tool and respond to user
...
- The dispatcher maintains a **skill routing table** (defined in `{{DISPATCHER_FILE}}`) with trigger phrases in multiple languages.
- If a user message matches a skill trigger, the skill is invoked via the **{{TOOL_SKILL}}** tool (not the `{{TOOL_AGENT}}` tool). The dispatcher does NOT also invoke the source agent.
...
See `{{PLATFORM_DIR}}/references/agents.md` (Skills section) for the full table of skills, their source agents, and purposes.
...
Custom agents are created by the Architect and stored in `{{PLATFORM_DIR}}/agents/`. They participate fully in the orchestration system:

1. **Creation**: the Architect creates the agent file, adds a row to `agents-registry.md`, and updates `agents.md`
2. **Discovery**: Claude Code auto-discovers the agent from its frontmatter in `{{PLATFORM_DIR}}/agents/`
...
5. **Maintenance**: the Librarian audits custom agents during vault health checks. For every row in agents-registry.md with status=active, the corresponding file must exist in `{{PLATFORM_DIR}}/agents/`
...
- ❌ **Do NOT edit other agents' prompt/config files** (e.g., `{{PLATFORM_DIR}}/agents/*.md`) — normal vault notes/MOC edits are still allowed per your responsibilities; all coordination goes through the dispatcher
...
- **Agent registry**: `{{PLATFORM_DIR}}/references/agents-registry.md` — the single source of truth for all agents
- **Agent directory**: `{{PLATFORM_DIR}}/references/agents.md` — detailed descriptions of each agent's responsibilities
