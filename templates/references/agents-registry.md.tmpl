...
This file is the **single source of truth** for all active agents in the crew. The dispatcher (`{{DISPATCHER_FILE}}`) and all agents reference this file for routing decisions and inter-agent coordination.
...
|------|------|-------------|-------|--------|--------|
| architect | Vault Structure & Governance | Create/modify folders, templates, MOCs, tag taxonomy, naming conventions. Full `{{TOOL_BASH}}` access. Runs onboarding. | Vault setup, new areas/projects, structural changes, defrag, onboarding | Folders created, templates defined, structure updated, MOCs generated | active |
...
| librarian | Vault Health & Quality Assurance | Detect/merge duplicates, fix broken links, audit frontmatter, growth analytics. Full `{{TOOL_BASH}}` access. | Maintenance, audit, cleanup, health check, duplicate detection | Health reports, fixed links, merged duplicates, consistency reports | active |
...
| postman | Email & Calendar Intelligence | Read/archive/delete email (Gmail via `gws`, Hey.com via `hey`), search emails, read/create/update calendar events, draft and send replies. Uses Google Workspace CLI (`gws`) and/or Hey CLI (`hey`) via `{{TOOL_BASH}}`, with MCP as read-only fallback. | Email triage, calendar queries, deadline tracking, meeting prep, VIP filtering | Email summaries saved as notes in `00-Inbox/`, calendar events created, deadline reports | active |
...
3. The Architect generates the agent file in `{{PLATFORM_DIR}}/agents/`, adds a row to the Registry table above, and updates `agents.md`
...
Skills handle complex, multi-step workflows extracted from agents. They are checked **before** agents by the dispatcher (higher priority). Skills run in the main conversation context via the `{{TOOL_SKILL}}` tool, preserving multi-turn state.
...
1. The dispatcher checks the **skill routing table** (in `{{DISPATCHER_FILE}}`) before the agent routing table
2. If a trigger matches, the skill is invoked via the **{{TOOL_SKILL}}** tool — not the `{{TOOL_AGENT}}` tool
