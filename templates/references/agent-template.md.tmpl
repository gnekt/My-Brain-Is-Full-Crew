...
#   {{TOOL_READ}}, {{TOOL_GLOB}}, {{TOOL_GREP}}        -> DEFAULT. Every agent gets these (search and read the vault)
#   {{TOOL_WRITE}}                   -> Only if the agent CREATES new notes or files
#   {{TOOL_EDIT}}                    -> Only if the agent MODIFIES existing notes or files
#   {{TOOL_BASH}}                    -> Only if the agent needs filesystem operations (move, rename, mkdir)
#                              or CLI tool access (e.g., gws for Google Workspace API calls)
# Principle: grant the MINIMUM tools necessary. Read-only agents should NOT have {{TOOL_WRITE}}/{{TOOL_EDIT}}.

model: {{MODEL_FAST}}
# Options: {{MODEL_FAST}} (default), {{MODEL_POWERFUL}} (deep reasoning), {{MODEL_LIGHT}} (fast/lightweight)
# Use {{MODEL_FAST}} unless there is a strong reason not to.
---
